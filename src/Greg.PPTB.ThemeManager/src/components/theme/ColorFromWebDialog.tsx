...
import { checkContrast, pickForeground, proposeTheme } from '../../model/colorRoles';
...
import { captureSite, ASSISTED_CAPTURE_INSTRUCTIONS } from '../../services/siteCapture';
...
const DEFAULT_STORED_OPTIONS: StoredOptions = { ignoreGreys: true, maxColors: 6, overrideSlots: false };
 ...
 */
export function ColorFromWebDialog({ open, onDismiss }: ColorFromWebDialogProps) {
    const styles = useStyles();
    const mountNode = usePortalMount();
    const { model, dispatch } = useThemeModel();

    const [options, setOptions] = usePersistedSetting<StoredOptions>('colorExtraction.options', DEFAULT_STORED_OPTIONS);
    ...
    const [headerBackground, setHeaderBackground] = useState<string | undefined>();
    const [headerForeground, setHeaderForeground] = useState<string | undefined>();
    ...
    const extractionOptions = useMemo<ExtractionOptions>(
        () => ({ ignoreGreys: options.ignoreGreys, maxColors: options.maxColors }),
        [options.ignoreGreys, options.maxColors],
    );
    ...
    const candidates = useMemo<ColorCandidate[]>(() => {
        ...
        return extractPalette(image.imageData, { ...extractionOptions, crop }).candidates;
    ...
    const headerCandidates = useMemo<ColorCandidate[]>(() => {
        ...
        return extractPalette(image.imageData, { ...extractionOptions, maxColors: 3, crop: crop ?? headerBandCrop(image.imageData) }).candidates;
    }, [image, extractionOptions, crop]);

    const proposal = useMemo(
        () => proposeTheme(candidates, headerCandidates, model, { overrideSlots: options.overrideSlots }),
    ...
    const toImagePoint = (event: { clientX: number; clientY: number }) => {
        ...
        return { x: Math.min(Math.max(x, 0), image.width - 1), y: Math.min(Math.max(y, 0), image.height - 1) };
    };
    ...
    const handlePointerUp = (event: React.PointerEvent<HTMLImageElement>) => {
        ...
        if (rect.width < 4 || rect.height < 4) {
            ...
            setCrop(undefined);
            const picked = image ? pixelAt(image.imageData, point.x, point.y) : undefined;
            if (picked) {
                setSeed(picked);
            }
            return;
    ...
    const effectiveForeground = headerForeground ?? (effectiveBackground ? pickForeground(effectiveBackground, candidates) : undefined);
    ...
    const handleApply = () => {
        dispatch({
            ...
            patch: {
                ...
                appHeaderForeground: effectiveBackground ? effectiveForeground : undefined,
                paletteOverrides: options.overrideSlots ? proposal.paletteOverrides : undefined,
            },
        });
        onDismiss();
    };
                ...
                <DialogBody>
                    ...
                    <DialogContent className={styles.body}>
                        ...
                        {step === 'source' && (
                            <>
                                <MessageBar intent="info">
                                    <MessageBarBody>
                                        <MessageBarTitle>Screenshots stay on this machine</MessageBarTitle>
                                        Power Platform ToolBox can't take a screenshot for you, so the site is opened in your browser and you paste the screenshot
                                        back here. Nothing is uploaded anywhere.
                                    </MessageBarBody>
                                </MessageBar>

                                <Field label="Website address" hint="Only http:// and https:// addresses can be opened.">
                                    <div className={styles.row}>
                                        ...
                                        />
                                        <Button appearance="secondary" icon={<OpenRegular />} disabled={busy || !url.trim()} onClick={handleOpenSite}>
                                            Open the site
                                        </Button>
                                    </div>
                                </Field>
                                ...
                                <div
                                    ...
                                    onDrop={(event) => {
                                        ...
                                        if (file) {
                                            ...
                                            setError('That item is not a supported image. Use a PNG, JPG, GIF, WEBP or BMP screenshot.');
                                        }
                                    }}
                                >
                                    ...
                                    <div className={styles.row}>
                                        <Button appearance="primary" disabled={busy} onClick={() => void loadImage(importImageFromFile)}>
                                            Browse for an image
                                        </Button>
                                        <Button appearance="secondary" disabled={busy} onClick={() => void loadImage(importImageFromClipboard)}>
                        ...
                        {step === 'image' && image && (
                            <>
                                <Text size={200} className={styles.hint}>
                                    Drag on the image to analyse only that region, or click a pixel to use its exact colour as the base palette color.
                                </Text>
                                ...
                                <div className={styles.row}>
                                    <Button size="small" appearance="secondary" onClick={() => setCrop(headerBandCrop(image.imageData))}>
                                        Header only
                                    </Button>
                                    <Button size="small" appearance="subtle" disabled={!crop} onClick={() => setCrop(undefined)}>
                                        Whole image
                                    </Button>
                                    <Switch
                                        label="Ignore greys"
                                        checked={options.ignoreGreys}
                                        onChange={(_, data) => setOptions({ ...options, ignoreGreys: data.checked })}
                                    />
                                    <Field label="Colors">
                                        <Input
                                            type="number"
                                            ...
                                            onChange={(_, data) => {
                                                ...
                                                if (Number.isFinite(parsed)) {
                                                    setOptions({ ...options, maxColors: Math.min(12, Math.max(1, Math.round(parsed))) });
                                                }
                                            }}
                                        />
                                    </Field>
                                ...
                                {candidates.length === 0 && (
                                    <MessageBar intent="warning">
                                        <MessageBarBody>
                                            No colour stands out in this area — it is all white, black or grey. Try another region, or turn "Ignore greys" off.
                                        </MessageBarBody>
                                    </MessageBar>
                                )}
                            </>
                            ...
                            <>
                                ...
                                <div className={styles.swatches}>
                                    {candidates.map((candidate) => (
                                        ...
                                        >
                                            <button type="button" className={styles.swatch} onClick={() => setSeed(candidate.hex)}>
                                                <span className={styles.swatchChip} style={{ backgroundColor: candidate.hex }} />
                                                <Text size={100}>{Math.round(candidate.coverage * 100)}%</Text>
                                            </button>
                                        </Tooltip>
                                    ))}
                                    {candidates.length === 0 && (
                                ...
                                />
                                <ColorField label="App header background" value={effectiveBackground} onChange={(value) => setHeaderBackground(value)} />
                                <ColorField
                                ...
                                {effectiveBackground && effectiveForeground && (
                                    <>
                                        ...
                                        <div className={styles.headerPreview} style={{ backgroundColor: effectiveBackground, color: effectiveForeground }}>
                                        ...
                                        {contrast && (
                                            <Text size={200} className={contrast.passes ? styles.pass : styles.fail}>
                                                Header contrast {contrast.ratio.toFixed(2)}:1 {contrast.passes ? 'meets' : 'is below'} the recommended{' '}
                                ...
                                <Switch
                                    ...
                                    onChange={(_, data) => setOptions({ ...options, overrideSlots: data.checked })}
                                />
                                <Text size={100} className={styles.hint}>
                                    Off by default: the generated ramp is the documented mechanism, and forcing screenshot colours into the 16 slots produces
                                    incoherent ramps.
                                </Text>
                            </>
                        )}
                    ...
                    <DialogActions>
                        {step !== 'source' && (
                            <Button appearance="secondary" onClick={() => setStep(step === 'roles' ? 'image' : 'source')}>
                                Back
                            </Button>
                        )}
                        ...
                        {step === 'image' && (
                            <Button appearance="primary" disabled={candidates.length === 0 && !seed} onClick={() => setStep('roles')}>
                                Next
                            </Button>
                        )}
                        {step === 'roles' && (
                            <Button appearance="primary" disabled={!canApply} onClick={handleApply}>
                                Apply
                            </Button>
                        )}
                    </DialogActions>
