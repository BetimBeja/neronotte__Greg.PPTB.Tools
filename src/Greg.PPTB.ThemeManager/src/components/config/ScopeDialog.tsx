...
export function ScopeDialog({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (otherDefinition) {
        const assignment = await readScopeAssignment(
          otherDefinition,
          otherKind,
        );
        const inUse =
  ...
  const handleApply = async () => {
    ...
    try {
      if (target === "environment") {
        ...
        setStatus(
          `"${THEME_SETTING_DISPLAY_NAMES[kind]}" is now set to ${webResourceName} for the whole environment. Publish all customizations to apply it.`,
        );
      } else if (appId) {
        await setAppScope(definition, appId, webResourceName);
        setStatus(
          `"${THEME_SETTING_DISPLAY_NAMES[kind]}" is now set to ${webResourceName} for the selected app. Publish all customizations to apply it.`,
        );
      } else {
          ...
          <DialogContent className={styles.body}>
            {!webResourceName && (
              <MessageBar intent="warning">
                <MessageBarBody>
                  Save the theme to a web resource first — the setting stores
                  its unique name.
                </MessageBarBody>
              </MessageBar>
            )}
            ...
            {webResourceName && (
              <div className={styles.row}>
                <Text size={200}>
                  Setting <strong>{THEME_SETTING_DISPLAY_NAMES[kind]}</strong> ={" "}
            ...
            {conflict && (
              <MessageBar intent="warning">
                <MessageBarBody>
                  ...
                  "Override app header color" is ignored by the platform
                  whenever "Custom theme definition" is set.
                </MessageBarBody>
              </MessageBar>
            )}
            ...
            {!scopeLoading && !apiAvailable && (
              <MessageBar intent="warning">
                <MessageBarBody>
                  ...
                  <strong> Add existing → More → Setting</strong>, pick{" "}
                  <strong>{THEME_SETTING_DISPLAY_NAMES[kind]}</strong>, paste
                  the unique name above as the setting value, and publish all
                  customizations.
                </MessageBarBody>
              </MessageBar>
            )}
              ...
              <>
                <Field label="Scope">
                  <RadioGroup
                    value={target}
                    onChange={(_, data) =>
                      setTarget(data.value as "environment" | "app")
                    }
                  >
            ...
            <Text size={100} className={styles.hint}>
              The theme settings are not covered by Microsoft's documented
              entity reference; the tool resolves them at runtime and never
              hardcodes ids.
            </Text>
          </DialogContent>
          <DialogActions>
