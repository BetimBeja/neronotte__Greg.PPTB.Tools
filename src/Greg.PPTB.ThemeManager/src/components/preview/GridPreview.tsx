...
import { Checkbox, Input, makeStyles, mergeClasses, shorthands, tokens } from '@fluentui/react-components';
import {
...
const VIEW_COMMANDS: CommandBarCommand[] = [
    ...
    { key: 'visualize', label: 'Visualize this view', icon: <ChartMultipleRegular /> },
];
...
export function GridPreview() {
    ...
    const rows = useMemo(() => {
        ...
        return SAMPLE_ACCOUNTS.filter((account) => Object.values(account).some((value) => value.toLowerCase().includes(needle)));
    }, [keyword]);
        ...
        <div className={styles.root}>
            ...
            <div className={styles.surface}>
                <div className={styles.gridHeader}>
                    <button type="button" className={styles.viewSelector} aria-label="View selector">
                        {SAMPLE_VIEWS[0]}
                        <ChevronDownRegular fontSize={20} />
                    </button>
                    <div className={styles.spacer} />
                    <button type="button" className={styles.iconButton} aria-label="Edit columns">
                        <TableSimpleRegular fontSize={20} />
                    </button>
                    <button type="button" className={styles.iconButton} aria-label="Edit filters">
                ...
                <table className={styles.table} data-themed="Row selection, hover and hyperlinks">
                    <thead>
                        <tr>
                            <th className={mergeClasses(styles.th, styles.checkboxCell)}>
                                <Checkbox
                                    ...
                                    onChange={(_, data) => setSelectedIds(data.checked ? rows.map((row) => row.id) : [])}
                                />
                            </th>
                            {COLUMNS.map((column) => (
                    ...
                    <tbody>
                        {rows.map((row) => {
                            ...
                            return (
                                <tr key={row.id} className={mergeClasses(styles.row, selected && styles.selectedRow)} aria-selected={selected}>
                                    <td className={mergeClasses(styles.td, styles.checkboxCell)}>
                                        <Checkbox checked={selected} aria-label={`Select ${row.name}`} onChange={() => toggleRow(row.id)} />
                                    </td>
                ...
                {rows.length === 0 && <div className={styles.empty}>No records match the keyword.</div>}
            </div>
        </div>
    );
}
