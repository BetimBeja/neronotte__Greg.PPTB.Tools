...
import { PALETTE_SLOTS, type AppHeaderColors, type PaletteSlot, type ThemeModel } from './theme';
import { applyPaletteOverrides, generateBrandRamp, hexToRgb, hslToRgb, rgbToHex, rgbToHsl } from './brandRamp';
...
function readableForeground(background: string): string {
    return relativeLuminance(background) > 0.4 ? DARK_FOREGROUND : LIGHT_FOREGROUND;
}
 ...
 */
export function resolveAppHeaderColors(colors: AppHeaderColors | undefined, brandPrimary: string): ResolvedAppHeaderColors {
    ...
    const foreground = colors?.foreground?.trim() || readableForeground(background);
    const backgroundHover = colors?.backgroundHover?.trim() || shiftLightness(background, direction * 8);
    const backgroundPressed = colors?.backgroundPressed?.trim() || shiftLightness(background, direction * 14);
    const backgroundSelected = colors?.backgroundSelected?.trim() || shiftLightness(background, direction * 20);

    return {
        background,
        foreground,
        backgroundHover,
        foregroundHover: colors?.foregroundHover?.trim() || readableForeground(backgroundHover),
        backgroundPressed,
        foregroundPressed: colors?.foregroundPressed?.trim() || readableForeground(backgroundPressed),
        backgroundSelected,
        foregroundSelected: colors?.foregroundSelected?.trim() || readableForeground(backgroundSelected),
    };
}
...
export function themeModelToPreviewTheme(model: ThemeModel): PreviewTheme {
    ...
    const fontFamily = model.font?.trim() || baseTheme.fontFamilyBase;

    return {
        fluentTheme: { ...baseTheme, fontFamilyBase: fontFamily },
        brand,
        headerColors: resolveAppHeaderColors(model.appHeaderColors, palette.primary),
        fontFamily,
    };
}
