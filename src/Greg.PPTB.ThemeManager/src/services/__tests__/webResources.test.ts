...
import { buildWebResourceName, imageMimeType, imageTypeFromFileName, validateWebResourceName, WEB_RESOURCE_TYPE } from '../webResources';
...
describe('web resource naming', () => {
    ...
    it('prepends the publisher prefix exactly once', () => {
        expect(buildWebResourceName('contoso', 'custom-theme')).toBe('contoso_custom-theme');
        expect(buildWebResourceName('contoso_', '_custom-theme')).toBe('contoso_custom-theme');
...
describe('image web resources', () => {
    ...
    it('builds a renderable data URI', () => {
        expect(toDataUri(WEB_RESOURCE_TYPE.png, 'AAA')).toBe('data:image/png;base64,AAA');
    ...
    it('suggests a safe web resource name from a file name', () => {
        expect(suggestLogoName('Company Logo (final).png')).toBe('Company-Logo-final');
        expect(suggestLogoName('.png')).toBe('app-logo');
    });
});
