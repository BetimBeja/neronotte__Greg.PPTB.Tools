...
import { DataverseOperationError, IMAGE_WEB_RESOURCE_TYPES, WEB_RESOURCE_TYPE, imageMimeType, imageTypeFromFileName, readWebResource } from './webResources';
...
const IMAGE_FILTERS = [{ name: 'Images', extensions: ['png', 'jpg', 'jpeg', 'gif', 'svg', 'ico'] }];
...
export function toDataUri(webResourceType: number, contentBase64: string): string {
 ...
 */
export function measureImage(dataUri: string): Promise<{ width: number; height: number } | undefined> {
...
export function logoSizeWarning(dimensions: { width: number; height: number } | undefined): string | undefined {
    if (!dimensions) {
        return undefined;
    }
    if (dimensions.width === RECOMMENDED_LOGO_SIZE.width && dimensions.height === RECOMMENDED_LOGO_SIZE.height) {
        return undefined;
    }
...
export async function pickLocalLogo(): Promise<LocalLogoFile | undefined> {
    ...
    if (webResourceType === undefined) {
        throw new DataverseOperationError(`"${fileName}" is not a supported image type. Use PNG, JPG, GIF, SVG or ICO.`);
    }
    ...
    const dimensions = webResourceType === WEB_RESOURCE_TYPE.ico ? undefined : await measureImage(dataUri);

    return { path, fileName, webResourceType, contentBase64, dataUri, dimensions };
}

/** Reads an image web resource and returns the `data:` URI for the preview. */
export async function readLogoDataUri(webResourceId: string): Promise<string | undefined> {
    const resource = await readWebResource(webResourceId);
    if (!IMAGE_WEB_RESOURCE_TYPES.includes(resource.webResourceType)) {
        throw new DataverseOperationError(`"${resource.name}" is not an image web resource.`);
    }
    return resource.contentBase64 ? toDataUri(resource.webResourceType, resource.contentBase64) : undefined;
}

/** Suggests a web resource local name from a file name. */
export function suggestLogoName(fileName: string): string {
    const withoutExtension = fileName.replace(/\.[^.]+$/, '');
    return withoutExtension.trim().replace(/[^A-Za-z0-9_.-]+/g, '-').replace(/^-+|-+$/g, '') || 'app-logo';
}
